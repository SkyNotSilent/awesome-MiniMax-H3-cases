...
describe('published prompt policy', () => {
  it('stores only verbatim published prompts and never prompt reconstructions or placeholders', () => {
    for (const item of rawCases) {
      expect(['official-verbatim', 'creator-verbatim', 'not-published', 'not-published']).toContain(item.promptProvenance)
      ...
      if (item.promptProvenance === 'not-published') {
        ...
        if (item.promptCompleteness === 'excerpt') expect(item.sourceUrl).toMatch(/^https:\/\//)
        if (item.promptProvenance === 'not-published') expect(item.sourceUrl).toMatch(/^https:\/\//)
      }
    }
  })
})
