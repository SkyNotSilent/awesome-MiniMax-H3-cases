...
export const copy = {
  zh: {
    ...
    catalog: {
      ...
      latestReleaseStatus: '最近更新',
      latestReleaseSummary: (date: string, summary: string) => `最近一次更新于 ${date} · ${summary}`,
      releaseHint: '按最近一次收录显示，每次发布后自动更新。',
      viewReleaseCases: (count: number) => `查看本次新增的 ${count} 个案例`,
      ...
      snapshotEmptyDescription: '可以查看近 7 天或完整案例库。',
      resetFilters: '清除其他筛选',
      viewAll: '查看全部',
      newlyAdded: '新收录',
    ...
    tutorials: {
      ...
      communityTitle: '社区教程',
      communityDescription: '按相对热度和可执行性精选，所有互动量均为公开快照；原文完整收录在站内，并保留作者署名与出处链接。',
      searchLabel: '搜索教程',
    ...
    creators: {
      ...
      methodology: '名次只基于本站已核验、可追溯的公开内容；不同平台的互动量不混合比较。',
      videoCreators: '视频创作者',
  ...
  en: {
    ...
    catalog: {
      ...
      collectionsExit: 'Leave quick collections',
      collections: {
        all: 'All',
      ...
      addedDatePresets: {
        release: 'Latest additions',
        '7d': 'Last 7 days',
        '30d': 'Last 30 days',
        all: 'All dates',
      },
      latestReleaseStatus: 'LATEST RELEASE',
      latestReleaseSummary: (date: string, summary: string) => `Latest release ${date} · ${summary}`,
      releaseHint: 'Shows the latest catalog release and refreshes with every publish.',
      viewReleaseCases: (count: number) => `View the ${count} newly added case${count === 1 ? '' : 's'}`,
      updateSummaryTitle: (cases: number, prompts: number, tutorials: number) => {
        const items = []
      ...
      snapshotEmptyDescription: 'Browse the last 7 days or the complete library instead.',
    ...
    tutorials: {
      ...
      communityDescription: 'Curated by relative reach and executability. Engagement figures are public snapshots; each original is reproduced in full with author credit and a source link.',
    ...
    creators: {
      ...
      methodology: 'Ranks use source-checked content in this library; engagement from different platforms is never compared directly.',
