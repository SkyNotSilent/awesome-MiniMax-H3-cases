...
const ui = {
  'zh-CN': {
    ...
    promptExcerptNotice: '以下为公开来源中以 ... 结尾的 Prompt 节选；未补写缺失内容。',
...
function renderCasePage(item, locale) {
  ...
  <p class="prompt-notice">${escapeHtml(item.promptCompleteness === 'excerpt' ? labels.promptExcerptNotice : labels.promptNotice)}${item.sourceUrl ? ` <a href="${escapeHtml(item.sourceUrl)}" rel="nofollow noopener">${escapeHtml(labels.archiveSource)} ↗</a>` : ''}</p>
