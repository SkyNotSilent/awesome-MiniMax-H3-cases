...
const skillInstallCommand = (item) => {
  ...
  if (!skill.path.includes('/')) return `npx skills add ${item.repository} --skill ${skill.name}`
...
function fallbackMarkup(page, locale) {
  ...
  if (page.id === 'catalog') {
    ...
    if (item.chapters?.length) content += `<h2>${heading('视频章节', 'Video chapters')}</h2><ul>${links(item.chapters.map(x => [language === 'zh' && x.urlZh ? x.urlZh : x.url, `${Math.floor(x.seconds / 60)}:${String(x.seconds % 60).padStart(2, '0')} · ${x.title[language]}`]))}</ul>`
