...
function englishModel(model) {
  return model
    ...
    .replaceAll('（公开来源标注）', ' (public-archive-labeled)')
...
function prebootMarkup(locale) {
  ...
  return `<aside class="preboot-splash" aria-label="MiniMax H3 Cases &amp; Guides">
      ...
      <div class="preboot-proof-card preboot-proof-rank"><small>${escapeHtml(updateEyebrow)}</small><strong>DAILY</strong><p>${escapeHtml(updateLabel)}</p><em>${escapeHtml(updateFootnote)}</em></div>
...
function renderCasePage(item, locale) {
  ...
  <p class="prompt-notice">${escapeHtml(labels.promptNotice)}${item.sourceUrl ? ` <a href="${escapeHtml(item.sourceUrl)}" rel="nofollow noopener">${escapeHtml(labels.archiveSource)} ↗</a>` : ''}</p>
...
<body><main>
  ...
  ${promptMarkup}
  <p><a href="${escapeHtml(item.sourceUrl)}" rel="nofollow noopener">${escapeHtml(labels.source)} · ${escapeHtml(copy.sourceLabel)}</a>${!hasPrompt && item.sourceUrl ? ` · <a href="${escapeHtml(item.sourceUrl)}" rel="nofollow noopener">${escapeHtml(labels.archiveSource)}</a>` : ''}</p>
