...
const provenance = new Set(['official-verbatim', 'creator-verbatim', 'not-published', 'not-published'])
...
for (const [index, item] of cases.entries()) {
  ...
  if (item.promptProvenance === 'not-published') {
    ...
    if (item.sourceUrl) errors.push(`${at}.sourceUrl is not allowed when promptProvenance is not-published`)
  ...
  if (item.promptCompleteness === 'excerpt' && !item.sourceUrl) errors.push(`${at}.sourceUrl is required for an archived prompt excerpt`)
  if (item.promptProvenance === 'not-published' && !item.sourceUrl) errors.push(`${at}.sourceUrl is required for external archive provenance`)
  ...
  if (item.sourceUrl) {
    try {
      const archiveSource = new URL(item.sourceUrl)
      if (archiveSource.protocol !== 'https:') errors.push(`${at}.sourceUrl must use HTTPS`)
    } catch {
      errors.push(`${at}.sourceUrl is invalid`)
    }
  }
