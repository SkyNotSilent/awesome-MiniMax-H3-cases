...
- Copy a prompt only when promptCompleteness is complete (or omitted for legacy records) and promptProvenance is official-verbatim, creator-verbatim, or not-published.
- Never publish or return a truncated Prompt excerpt.
- When promptProvenance is not-published, cite sourceUrl and do not claim that the original X post was re-verified.
...
${item.sourceUrl ? `- Archive source: ${item.sourceUrl}` : ''}
