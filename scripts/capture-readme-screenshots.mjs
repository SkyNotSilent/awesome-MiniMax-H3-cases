...
async function focusTutorialUpdates(page) {
  const tracks = page.locator('.tutorial-track-grid')
  await tracks.waitFor()
      ...
      *{box-sizing:border-box} body{margin:0;background:#090a08;color:#f4f5ed;font-family:ui-monospace,SFMono-Regular,Menlo,monospace}
      ...
      <div class="grid">
        ...
        <section class="panel"><div class="label">02 / Structured tutorial result</div><h2>${escapeHtml(guide.title.en)}</h2>
          ...
          <div class="row"><span>DIFFICULTY / TIME</span><strong>${escapeHtml(guide.difficulty)} · ${guide.estimatedMinutes ? `about ${escapeHtml(guide.estimatedMinutes)} minutes` : 'runtime depends on hardware'}</strong></div>
          <div class="row"><span>APPLICABLE VERSIONS</span><strong>${escapeHtml(guide.applicableVersions.join(' · '))}</strong></div>
