...
Retrieve source-attributed MiniMax H3 / Hailuo 3.0 video cases from the catalog. Return prompt text only when the original creator post or an official public script published that exact text.
...
5. Return a prompt only when provenance is `official-verbatim` or `creator-verbatim` and the source supports the exact text.
6. Treat `unknown`, `official-adapted`, `reconstructed`, or any unrecognized provenance as unavailable. Do not present stored text under those labels as an original prompt.
7. Preserve public prompt wording, order, punctuation, timestamps, formatting, and language exactly. Do not clean, translate, shorten, expand, or normalize it.
...
When a verbatim public prompt exists, reproduce only that exact prompt and link the original source.
