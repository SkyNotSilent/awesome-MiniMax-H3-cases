...
5. Return a complete prompt only when `promptCompleteness` is `complete` (or omitted for legacy records) and provenance is `official-verbatim`, `creator-verbatim`, or `not-published`.
6. When `promptCompleteness` is `excerpt`, label it as an incomplete archived excerpt, preserve the trailing ellipsis, and cite `sourceUrl`; never present it as complete.
7. For `not-published`, disclose that the original post was unavailable during recheck and cite the archive source instead of claiming creator re-verification.
