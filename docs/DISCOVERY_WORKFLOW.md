...
6. Classify mode, category, style, scene, and input types using public post metadata and visible media. Category, styles, and scenes must use keys from `data/taxonomy.json`; styles and scenes are independent arrays with at most two values. Keep classifications factual and bounded by the available evidence. If category evidence is insufficient, use `showcase`; if style or scene evidence is insufficient, use empty arrays rather than inferred filler values. Never infer `mode` from appearance.
...
11. Run `npm run classify` with MiMo V2.5 Pro when `MIMO_API_KEY` is available. The classifier reads the fixed vocabulary from `data/taxonomy.json`, rejects values outside it, and stores its short category/style/scene evidence only in `.review/`. Use it only to classify public text and metadata; do not ask it to produce prompt text.
12. Send only ambiguous, shortlisted, or evidence-poor `showcase` videos to `mimo-v2.5` for low-FPS verification. Eligibility review has priority; remaining daily taxonomy-review capacity may refine category, styles, and scenes. The same vocabulary, two-value limit, and empty arrays rule apply. Do not ask for prompt reconstruction, mode inference, or workflow decomposition.
...
Use the private operations repository's `daily-run.mjs` entry point with an explicit public checkout and run ID. The implementation is available for integration; enabling it in the actual scheduled task is a separate operation. Begin from a verified discovery watermark, overlap six hours, retain the first incomplete window and cursor, and split longer gaps into continuous daily windows. Without trustworthy earlier evidence, bootstrap 48 hours and label earlier coverage unknown. An unresolved backlog over seven days requires attention; it must not be skipped or used to increase daily limits. Reserve at least half the existing discovery allowance for open search.

Discovery completion and publication completion are separate. Attach timestamped access evidence and a content hash to each window; login failure, throttling, a timer firing or an agent's prose is not a completed discovery window. Validate public changes with `npm run verify`. A run with no public changes and all required evidence is `no-change`; do not create an empty commit or deployment. A changed run remains partial until the exact commit's CI, deployment and online checks agree. Notify once on completion, failure or required action; unchanged blocked state does not need repeated messages. A sleeping/offline Mac cannot send an immediate failure notification.

Publication now holds the shared writer lock before reading public and private state. The private staging journal records prepared, public-written and complete phases. An interrupted apply can be rerun with the same staging path; it deduplicates already published IDs and reconciles candidates. Completed manifests remain empty for idempotent retries. Media writeback, taxonomy migration and private featured selection use the same lock. Active or unknown owners are never stolen; inspect `node scripts/recover-publish-lock.mjs` before any recovery. Only an explicitly requested recovery of a confirmed dead process on this host is supported. Public posters are additive; no recovery removes published content or media.

README values are generated from `npm run build:reference` using the pinned Node release and no private analytics configuration. Use `npm run readme:build-metrics` before committing; reuse that build for required screenshots, then run `npm run verify`. CI checks the committed generated output; it never silently rewrites stale documentation. Actual deployment activation must verify Railway's Wait for CI for the same commit; direct deploys must not bypass a failed or pending check.
