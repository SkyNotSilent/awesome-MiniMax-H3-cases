...
10. Automatically publish a post when the original caption explicitly attributes the native video to MiniMax H3/Hailuo 3.0, the author and source are clear, the media is playable, and it is not an obvious repost, bulk advertisement, news/tutorial clip, or unrelated output. Download the highest-quality public MP4, upload it to the project's private Railway bucket, set `mediaUrl` to `/media/{case-id}.mp4`, cache its poster, add it to `data/cases.json`, and remove it from the candidate queue. Preserve `sourceUrl` so the case player can keep its X source icon.
...
13. Keep only ambiguous, incomplete, or disputed items in `data/candidates.json` with a factual review note. Clear cases do not require a second manual confirmation.
